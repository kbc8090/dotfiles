...
let s:p.normal.left     = [ [ s:p_black, s:p_purple, 'bold' ], [ s:p_purple, s:p_menu_grey ], [ s:p_purple, s:p_cursor_grey ] ]
...
let s:p.insert.left     = [ [ s:p_black, s:p_blue, 'bold' ], [ s:p_blue, s:p_menu_grey ], [ s:p_blue, s:p_cursor_grey ] ]
let s:p.insert.right    = copy(s:p.insert.left)
let s:p.insert.middle   = copy(s:p.normal.middle)

let s:p.visual.left     = [ [ s:p_black, s:p_cyan, 'bold' ], [ s:p_cyan, s:p_menu_grey ], [ s:p_cyan, s:p_cursor_grey ]]
let s:p.visual.right    = copy(s:p.visual.left)
let s:p.visual.middle   = copy(s:p.normal.middle)

let s:p.replace.left    = [ [ s:p_black, s:p_red, 'bold' ], [ s:p_red, s:p_menu_grey ], [ s:p_red, s:p_cursor_grey ]]
let s:p.replace.right   = copy(s:p.replace.left)
let s:p.replace.middle  = copy(s:p.normal.middle)

let s:p.tabline.left    = [ [ s:p_comment_grey, s:p_black ] ]
let s:p.tabline.right   = [ [ s:p_black, s:p_green, 'bold' ] ]
let s:p.tabline.middle  = [ [ s:p_black, s:p_menu_grey ] ]
let s:p.tabline.tabsel  = [ [ s:p_menu_grey, s:p_yellow, 'bold' ] ]
